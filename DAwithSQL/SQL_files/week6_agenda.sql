...
;with temp_cte
as(
select ss.[store_name], pp.[product_name], sum(ps.[quantity]) as stock_quantity,
row_number() over(partition by ss.[store_name] order by sum(ps.[quantity]) asc) as least_3
...
;with temp2_cte
as(
select s.city, p.[product_name], datename(year, o.[order_date]) as order_date, sum(i.[quantity]) as sold_quantity,
row_number() over(partition by s.city order by sum(i.[quantity]) asc) as least_3
...
group by s.city, p.[product_name], datename(year, o.[order_date])
)
