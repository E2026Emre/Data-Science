...
select distinct [rev_name], [mov_title], [rev_stars]
...
--that has at least one rating and find the highest number of stars that movie received and
...
join [dbo].[actor] a
on a.[act_id]=mc.[act_id]
group by [mov_title],[act_fname],[act_lname],[role]
order by [mov_title];
...
select top 1 [mov_title],[mov_year],rev_stars,[mov_rel_country]
from [dbo].[movie] m
join [dbo].[rating] ra
...
select top 1 [mov_title],[mov_year],rev_stars,[gen_title]
from [dbo].[movie] m
...
select distinct [mov_year], [gen_title],
				count([mov_title]) over(partition by [mov_year]) as num_of_movies,
