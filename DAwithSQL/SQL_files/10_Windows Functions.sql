...
select p.FIRSTNAME,p.[WEIGHT],
		lead(p.[WEIGHT],1,-1) over (order by p.[WEIGHT]) [lead 1 weight],
		lag(p.[WEIGHT],2,-1)  over (order by p.[WEIGHT]) [lag 2  weight]
from [dbo].[PERSON] p
order by p.[WEIGHT]
...
select p.FIRSTNAME,p.HEIGHT,
		CUME_DIST() over (order by p.[HEIGHT]) [cume dist height],
		PERCENTILE_DISC(.50) WITHIN GROUP (order by p.[HEIGHT]) over () [percnt dist 50],
...
-- Compute the height associated with the percentiles .50 and .72.
select  p.FIRSTNAME,p.HEIGHT,
        PERCENTILE_CONT(0.50) within group (partition by p.[GENDER] order by p.[HEIGHT]) over () [percnt dist 50],
		PERCENTILE_CONT(0.72) within group (partition by p.[GENDER] order by p.[HEIGHT]) over () [percnt dist 72]
from [dbo].[PERSON] p
order by p.HEIGHT
