...
SELECT	DISTINCT list_price, product_id, product_name
FROM	production.products
ORDER BY list_price DESC
OFFSET	1 ROWS
FETCH NEXT 1 ROWS ONLY;
...
SELECT	*
FROM	sales.orders
WHERE	YEAR(order_date) = YEAR(shipped_date) - 1;
...
SELECT	*,	CASE
				...
				WHEN DATEDIFF(DAY, order_date, shipped_date) IS NULL THEN 'tarih hatalı'
				ELSE 'çok yavaş'
			END AS shipping_speed
FROM	sales.orders
ORDER BY shipped_date DESC;
...
-- Herbir item için toplam tutarı (adet sayısı ve discount raito kullanılarak) hesaplayınız
